...
@section('content')
    <div class="md:flex items-start justify-center bg-gray-100 p-4 px-10 flex-col">
        <div class="w-full mx-auto px-2 mb-4">
            <div class="flex items-center space-x-2 text-gray-400 text-sm">
                ...
                <a href="/store/hotel"
                    class="hover:underline hover:text-gray-600 hover:scale-110 transition duration-300">Hoteles</a>
                <span>
                    <svg class="h-5 w-5 leading-none text-gray-300" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        ...
        <div class="md:flex items-start justify-center py-12 2xl:px-20 md:px-6 px-4 bg-white shadow-lg rounded-xl">
            <div class="w-2/4 md:block hidden">
                <img class="w-full rounded-xl" alt="Hotel"
                    src="https://images.unsplash.com/photo-1589923158776-cb4485d99fd6?q=80&w=1964&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" />
            </div>
            <div class=" w-full ml-6">
                ...
                <div class="py-4 border-b border-gray-200 flex items-center justify-between">
                    <p class="text-base leading-4">Ciudad</p>
                    <div class="flex items-center justify-center">
                        {{-- <p class="text-sm leading-none">{{ $prod->city_id }}</p> --}}
                        <p class="text-sm leading-none">
                            {{ $extra->name }}, {{ $extra->country->name }}
                        </p>
                    </div>
                </div>
                <div class="py-4 border-b border-gray-200 flex items-center justify-between">
                    <p class="text-base leading-4">Fechas</p>
                    <div class="flex items-end justify-center flex-col">
                        {{-- <p class="text-sm leading-none">{{ $prod->city_id }}</p> --}}
                        <p class="text-sm leading-none">
                            <span class="font-semibold">Entrada:</span> {{ $prod->entrance_date }}
                        </p>
                        <p class="text-sm leading-none mt-2">
                            <span class="font-semibold">Salida:</span> {{ $prod->leave_date }}
                        </p>
                    </div>
                </div>
                <div class="py-4 border-b border-gray-200 flex items-center justify-between">
                    <p class="text-base leading-4">Amenities</p>
                    <div class="flex items-start justify-start flex-col">
                        @foreach (explode(',', $prod->amenities) as $service)
                            <p class="text-sm leading-none mt-2">&#149; {{ $service }}</p>
                        @endforeach
                    </div>
                </div>
                @auth
        ...
        <div class='flex items-center justify-center w-full mt-10 flex-col gap-4'>
            ...
            @foreach ($reviews as $review)
                ...
                @elseif (!Auth::guest() and Auth::user()->role == 'admin')
                    ...
                    @else
                        <div class="rounded-xl border p-5 shadow-md w-full bg-slate-200">
                            <div class="flex w-full items-center justify-between border-b border-black pb-3">
                                <div class="flex items-center space-x-3">
                                    <div class="h-8 w-8 rounded-full bg-slate-400 bg-[url('https://i.pravatar.cc/33')]">
                                    </div>
                                    <div class="text-lg font-bold text-slate-700">{{ $review->user->name }}</div>
